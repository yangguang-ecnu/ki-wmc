...
public:
  ...
  void addImage(InputImagePointerType inputImage) {
    this->imgs.push_back(inputImage);
  }

  void makeReady(unsigned int rad) {
    ...
    logMessage << "Accessor is ready (" << this->numberOfInputs
        << " images), total number of available pixels is: "
        << this->total_pix << " (" << this->l << " DIM)\n";
    itk::OutputWindowDisplayText(logMessage.str().c_str());
  }

  void calc_index(size_t p_index, typename InputImageType::IndexType& index) {
    size_t mul = 1;
    for (unsigned int i = 0; i < InputImageType::ImageDimension; i++) {
  ...
  const double dot(size_t i1, size_t i2) {
    double d = 0;
  ...
  const double norm2(size_t index) {
    double n = 0;
  ...
  const std::vector<double> get(size_t index) {
  ...
  const void printLoc(size_t index) {
    img_index = index / this->image_pix;
    calc_index(index - img_index * this->image_pix, indx);
    std::cout << "LinearIndex: " << index << std::endl;
    std::cout << "ImageIndex: " << img_index << std::endl;
    std::cout << InputImageType::ImageDimension << "DIndex: " << indx << std::endl;
  }
  ...
  virtual ~ImageNeighborLinearAccess() {
